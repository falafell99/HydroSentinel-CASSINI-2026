...
function Panel({ title, defaultOpen = false, children }) {
  ...
  return (
    <div className="card" style={{ borderRadius: 8, overflow: 'hidden' }}>
      ...
      >
        {title}
        <span style={{ transition: 'transform 0.2s', transform: open ? 'rotate(0deg)' : 'rotate(-90deg)', color: open ? 'var(--teal)' : 'var(--text-muted)' }}>
          <ChevronDown size={16} />
        </span>
      </button>
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, alignItems: 'center' }}>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--teal)', fontWeight: 600 }}>{val}{unit}</span>
      </div>
      <input
        ...
        style={{
          ...
          background: `linear-gradient(to right, var(--teal) ${val}%, var(--border) ${val}%)`,
          borderRadius: 2, outline: 'none', cursor: 'pointer', accentColor: 'var(--teal)',
        }}
      />
...
function Toggle({ label, defaultChecked = false, disabled = false, note }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, opacity: disabled ? 0.5 : 1 }}>
      <div>
...
/**
 * SettingsTab — Fully functional accordion with all 5 sections
 */
export default function SettingsTab() {
  const [selectedRefresh, setSelectedRefresh] = useState(1); // "Every hour"

  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12, background: 'var(--bg-base)', animation: 'fadeSlideIn 0.25s ease-out' }}>
      {/* Page title */}
      ...
      {/* ── Section 1: Inspector Profile (open by default) ── */}
      <Panel title="Inspector Profile" defaultOpen>
        ...
        <button className="btn-primary" style={{ width: 'auto', padding: '9px 20px' }}>Save Profile</button>
      </Panel>
      ...
      <Panel title="API Connections">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {API_CARDS.map(({ status, name, desc, latency, hasKeyInput }) => (
            <div key={name} className="card" style={{ padding: '12px 16px', borderRadius: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button className="btn-outline" style={{ width: 'auto', padding: '5px 12px', fontSize: 12, flexShrink: 0 }}>Configure</button>
              </div>
              {hasKeyInput && (
                <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                  <input
                    placeholder="Enter Sentinel Hub API key…"
                    style={{ flex: 1, padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-primary)', background: 'var(--bg-elevated)', outline: 'none' }}
                  ...
                  />
                  <button className="btn-teal-outline" style={{ padding: '8px 14px', fontSize: 12 }}>Verify key</button>
                </div>
              )}
            </div>
          ))}
      ...
      <Panel title="Data & Refresh Rate">
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 2, background: 'var(--bg-elevated)', borderRadius: 8, padding: 4, width: 'fit-content' }}>
            {REFRESH_OPTS.map((opt, i) => (
              <button
                ...
                style={{
                  ...
                  border: 'none', fontFamily: 'var(--font-ui)', fontSize: 12, fontWeight: selectedRefresh === i ? 600 : 400,
      ...
      {/* ── Data Source Registry (always visible) ── */}
      <div className="card" style={{ padding: '16px 18px', background: 'var(--bg-elevated)', borderRadius: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '8px 16px' }}>
          {Object.entries(DATA_SOURCES).map(([key, val]) => (
            <React.Fragment key={key}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)', textTransform: 'uppercase' }}>
